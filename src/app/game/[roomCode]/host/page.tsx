  ...
  export default function HostGamePage() {
    ...
    useEffect(() => {
      if (gameRoom?.chaser_type) {
        console.log("Updating chaserType from gameRoom:", gameRoom.chaser_type);
    ...
    const handleZombieAttack = useCallback((playerId: string, newHealth: number, newSpeed: number) => {
      console.log(`🧟 Memulai serangan pada pemain ${playerId}! Kesehatan: ${newHealth}, Kecepatan: ${newSpeed}, chaserType: ${chaserType}`);
      ...
      attackIntervalRef.current = setInterval(() => {
        ...
        if (progress >= 0.5 && progress < 0.52) {
          setBackgroundFlash(true);
          console.log("💥 Pengejar mengenai!");
        }

        if (progress >= 1) {
          console.log(`✅ Serangan selesai untuk pemain ${playerId}, chaserType: ${chaserType}`);
          if (attackIntervalRef.current) {
            clearInterval(attackIntervalRef.current);
            attackIntervalRef.current = null;
          }
          ...
          setAttackQueue((prev) => {
            ...
            if (nextQueue.length > 0) {
              ...
              if (nextState && nextState.speed <= 30 && nextState.health > 0 && nextPlayer?.is_alive) {
                console.log(`📋 Memproses antrian berikutnya: ${nextPlayerId}, chaserType: ${chaserType}`);
                supabase
                  .from("player_health_states")
                  .update({
      ...
      setTimeout(() => {
        if (attackIntervalRef.current && zombieState.isAttacking) {
          console.log(`🛑 Timeout cadangan: Memaksa reset serangan untuk ${playerId}, chaserType: ${chaserType}`);
          clearInterval(attackIntervalRef.current);
          attackIntervalRef.current = null;
          setZombieState({
            isAttacking: false,
    ...
    }, [zombieState, playerStates, players, gameRoom, chaserType]);

    // Menangani jawaban benar
    const handleCorrectAnswer = useCallback((playerId: string, newSpeed: number) => {
      console.log(`✅ Pemain ${playerId} menjawab benar! Kecepatan: ${newSpeed}, chaserType: ${chaserType}`);
      setPlayerStates((prev) => ({
        ...prev,
        [playerId]: {
          ...prev[playerId],
      ...
      // Hanya hentikan serangan jika pemain yang menjawab benar sedang diserang
      if (zombieState.targetPlayerId === playerId && zombieState.isAttacking) {
        console.log(`🛑 Menghentikan serangan pengejar untuk ${playerId} karena jawaban benar, chaserType: ${chaserType}`);
        if (attackIntervalRef.current) {
          clearInterval(attackIntervalRef.current);
          attackIntervalRef.current = null;
        }
        ...
        setAttackQueue((prev) => {
          ...
          if (nextQueue.length > 0) {
            ...
            const nextPlayer = players.find((p) => p.id === nextPlayerId);
            if (nextState && nextState.speed <= 30 && nextState.health > 0 && nextPlayer?.is_alive) {
              console.log(`📋 Memproses antrian berikutnya setelah jawaban benar: ${nextPlayerId}, chaserType: ${chaserType}`);
              supabase
                .from("player_health_states")
                .update({
                  health: Math.max(0, nextState.health - 1),
    ...
    }, [zombieState, playerStates, players, gameRoom, chaserType, handleZombieAttack]);

    // Memperbarui countdown untuk pemain
    const updateCountdown = useCallback(() => {
      ...
      setPlayerStates((prev) => {
        ...
        Object.entries(updatedStates).forEach(([playerId, state]) => {
          ...
          if (newCountdown <= 0) {
            if (zombieState.isAttacking) {
              console.log(`⚠️ Pengejar sedang menyerang ${zombieState.targetPlayerId}, menunda serangan untuk ${playerId}`);
            ...
            console.log(`🧟 Memulai serangan pada ${playerId}! Kesehatan: ${newHealth}, Kecepatan: ${state.speed}, chaserType: ${chaserType}`);
    ...
    }, [gameRoom, playerStates, zombieState, handleZombieAttack, attackQueue, players, chaserType]);

    // Memeriksa pemain dengan kecepatan rendah
    const checkLowSpeedPlayers = useCallback(() => {
      if (!gameRoom) {
        console.log("⚠️ gameRoom kosong, lewati pemeriksaan kecepatan rendah");
        return;
      }
      console.log("🔍 Memeriksa pemain dengan kecepatan rendah:", { playerStates, zombieState, attackQueue, chaserType });
      ...
      eligiblePlayers.forEach(([playerId, state]) => {
        if (!attackQueue.includes(playerId)) {
          console.log(`📋 Menambahkan ${playerId} ke antrian serangan (Kecepatan: ${state.speed}, chaserType: ${chaserType})`);
    ...
    }, [playerStates, players, attackQueue, gameRoom, chaserType]);

    // Memeriksa penalti ketidakaktifan
    ...
    const fetchGameData = useCallback(async () => {
      try {
        ...
        console.log("Fetched room data:", { ...room, chaser_type: room.chaser_type });
        setGameRoom(room);
    ...
    useEffect(() => {
      ...
      const roomChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            console.log("🏷️ Perubahan game_rooms terdeteksi:", { ...payload.new, chaser_type: (payload.new as GameRoom).chaser_type });
          ...
          (payload) => {
            ...
            if (healthState) {
              console.log(`🔄 Memperbarui status untuk ${healthState.player_id}:`, {
                ...
                is_being_attacked: healthState.is_being_attacked,
              });
              ...
              if (healthState.speed <= 30 && !healthState.is_being_attacked && !zombieState.isAttacking && healthState.health > 0) {
                console.log(`🧟 Menambahkan ${healthState.player_id} ke antrian serangan dari Supabase, chaserType: ${chaserType}`);
                setAttackQueue((prev) => {
        ...
        .on(
          ...
          (payload) => {
            ...
            if (answer.is_correct) {
              console.log(`✅ ${playerName} menjawab benar! Kecepatan: ${answer.speed || 20}, chaserType: ${chaserType}`);
              handleCorrectAnswer(answer.player_id, answer.speed || 20);
            } else {
              console.log(`❌ ${playerName} menjawab salah! Pengejar akan menyerang jika kecepatan <= 30..., chaserType: ${chaserType}`);
            }
          }
        )
        .on(
    ...
    }, [gameRoom, handleZombieAttack, handleCorrectAnswer, fetchGameData, players, zombieState.isAttacking, router, roomCode, chaserType]);
    ...
    // Memeriksa kecepatan rendah dan penalti ketidakaktifan
    useEffect(() => {
      if (!gameRoom) return;
    ...
    // Mengatur pembersihan serangan terbaru
    useEffect(() => {
      const interval = setInterval(() => {
    ...
    if (!isClient || isLoading) {
      return (
        <div className="relative w-full h-screen bg-black flex items-center justify-center">
          <div className="text-white text-xl">Memuat Kejaran Pengejar...</div>
        </div>
      );
    }
